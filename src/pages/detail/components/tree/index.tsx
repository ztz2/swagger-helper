...
import { ProjectModuleInterface } from '@/core/types';
...
const DetailPage: FC<DetailPageProps> = ({ items, onSelect, key = 'uid' }) => {
  ...
  const handleSubSelect = (
    rootNode: ProjectModuleInterface,
    uid: string,
    e: Event,
  ) => {
    e.stopPropagation();
    ...
    const res = selectSubKeys
      .map((k) => rootNode.apiList.find((t) => t.uid === k))
      .filter((t) => t);
    // 执行回调
    onSelect(res);
  };
      ...
      {items.map((item) => (
        ...
        <div className={styles.treeItem} key={item[key]}>
          <div
            className={[
              styles.treeLabel,
              selectRootKey === item.uid || selectSubParentKey === item.uid
                ? styles.treeLabelActive
                : null,
            ].join(' ')}
          >
            <div>
              <div
                ...
                onClick={(e: Event) => handleRootSelect(item, e)}
              >
                <span>{item.label}</span>
                {item.description && (
                  <span
                    className="f10 text-ellipsis"
                    style={{
                      fontWeight: '400',
                      marginTop: '0px',
              ...
              <div>
                {selectSubParentKey === item.uid &&
                  selectSubKeys.length > 0 && <div>{selectSubKeys.length}</div>}
              </div>
            </div>
            {item.apiList && item.apiList.length > 0 && (
              <div
                className={
                  expandUidList.includes(item.uid)
                    ? styles.treeItemExpand
                    : null
                }
              ...
              <div className={styles.treeContent}>
                {item.apiList.map((t) => (
                  ...
                  <div
                    ...
                    className={[
                      styles.treeSub,
                      selectSubKeys.includes(t.uid)
                        ? styles.treeSubActive
                        : null,
                    ].join(' ')}
